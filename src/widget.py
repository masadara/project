...
def mask_account_card(account: Union[str] = '0') -> str:
    ...
    if isinstance(account, str):
        count = re.search(r"\d", account).start()
        if len(account) == 1:
            return 'error'
        if "Счет" in account:
            mask_account = account[:5] + "*" * 2 + account[-4:]
        else:
            mask_account = account[:6 + count] + "*" * 6 + account[12 + count:]
            mask_account = (
                    mask_account[:4 + count] + " " + mask_account[4 + count:8 + count] + " " + mask_account[8 + count:12 + count] + " " + mask_account[12 + count:]
            )
...
if __name__ == "__main__":
    print(mask_account_card("American Express 9496683068632548"))
